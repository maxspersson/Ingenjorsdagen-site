...
export default function SiteHeader() {
  ...
  return (
    <nav
  className={`fixed top-0 left-0 right-0 z-50 w-full text-[#1f1f1f] transition-[background-color,box-shadow,backdrop-filter] duration-300 ${
        scrolled
  ? "bg-white/50 backdrop-blur-lg shadow-[0_6px_20px_rgba(0,0,0,0.06)]"
  : "bg-white"
      }`}
    >
      <div
        className={`flex items-center justify-center overflow-hidden px-6 transition-[height,padding] duration-300 ${
          scrolled
            ? "h-[42px] sm:h-[48px] md:h-[56px]"
